...
// referrals coordinator) as the ירוקים (הפניות) actual for a month. The
// report has no division column at all -- every row is just a customer + a
// closed quote -- so, like "מוצרים יפה", this lands as one company-wide row
// in rapid_sales_categories (division=null), not split per division.
//
// Only rows whose תאריך יצירה falls in the target month AND סגורה=True
// count -- an open/unclosed quote isn't realized revenue yet. Sums תקבולים
// (amount actually RECEIVED so far), not סך הכל לאחר הנחה (the full closed
// deal value) -- a closed deal can be paid in installments, so the deal
// total can overstate what's actually landed this month (confirmed with
// Meital 2026-08-12). Falls back to סך הכל לאחר הנחה for older report
...
// but this is what the "עדכון ירוקים" upload button in the dashboard calls.
...
export type ParsedReferrals = {
  ...
  excluded: { otherMonth: number; notClosed: number };
  totalRows: number;
  amountSource: string;
};
...
function rowMonth(dateStr: unknown): string | null {
...
export function parseReferralsReport(buffer: Buffer, targetMonth: string): ParsedReferrals {
  ...
  return { targetMonth, amount, includedCount, excluded, totalRows: dataRows.length, amountSource };
}

export async function applyReferralsImport(supabase: SupabaseClient, parsed: ParsedReferrals) {
  const month = `${parsed.targetMonth}-01`;
